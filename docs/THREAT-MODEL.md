...
| An agent's own report | The least reliable signal in the system. Its STATUS word can lower a grade and never raise one (`gradeGates` in `src/lib/close.mjs`). The library also has three refusals that would stop a close outright on a report with no STATUS word or a DONE with no `Evidence:` line (`doneReportRefusal`), but the shipped close driver calls it without the flag that says a report is present, so none of them fires today. `docs/gates.json` records this under `close_refusals`, and its test turns red when the call is fixed. |
